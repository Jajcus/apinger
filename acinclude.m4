...
AC_CHECK_MEMBERS([struct icmphdr.type, struct icmphdr.code,\
struct icmphdr.checksum, struct icmphdr.un.echo.sequence,\
struct icmphdr.un.echo.id],[],
...
AC_CHECK_HEADERS([netinet/ip6.h],[jk_icmp_includes="$jk_icmp6_includes
#include <netinet/ip6.h>"],,[$jk_inet_includes])
AC_CHECK_HEADERS([netinet/icmp6.h],[jk_icmp_includes="$jk_icmp6_includes
...
struct icmp6_hdr.icmp6_id],,[HAVE_IPV6=no],[$jk_icmp_includes])
fi
